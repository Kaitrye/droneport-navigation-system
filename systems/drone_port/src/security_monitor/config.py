"""Configuration for the DronePort security monitor."""
...
def component_topic() -> str:
    return (
        ...
        or os.environ.get("DRONE_PORT_SECURITY_MONITOR_TOPIC")
...
def journal_file_path() -> str:
    return (
        ...
        or "/var/log/drones/drone_port_security_journal.ndjson"
    ).strip()
...
def service_name() -> str:
    return (os.environ.get("SECURITY_JOURNAL_SERVICE_NAME") or "DronePort").strip()


def service_id() -> int:
    raw = (os.environ.get("SECURITY_JOURNAL_SERVICE_ID") or "2").strip()
    try:
        value = int(raw)
    except ValueError:
        return 2
    return value if 1 <= value <= 1000 else 2
